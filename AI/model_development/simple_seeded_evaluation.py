...
class SimpleSeededEvaluator:
    """Simple evaluation using existing metrics calculator with different seeds."""
    ...
    def __init__(self):
        ...
        self.classifier_path = self.output_dir / "phase4_best_model.pth"
    ...
    def evaluate_with_seed(self, seed):
        """Evaluate models with given seed using existing metrics calculator."""
        ...
        try:
            ...
            # Initialize metrics calculator directly
            metrics_calculator = TwoStageMetricsCalculator()
            
            # Check if models exist
            if not self.model_path.exists():
                logger.error(f"❌ Autoencoder not found: {self.model_path}")
                return None
            ...
            # Run evaluation with current seed
            ...
            metrics = metrics_calculator.calculate_two_stage_metrics(
                ...
                test_data_path=None  # Let it find default test data
            )
    ...
    def save_results(self):
        ...
        json_file = self.output_dir / f"simple_seeded_evaluation_detailed_{timestamp}.json"
        with open(json_file, 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        logger.info(f"💾 Detailed results saved to: {json_file}")
        
        # Save summary as CSV
        csv_file = self.output_dir / f"simple_seeded_evaluation_summary_{timestamp}.csv"
...
def main():
    ...
    logger.info("🎲 Simple Seeded Evaluation for Two-Stage Anomaly Detection")
    logger.info("="*60)
    ...
    evaluator = SimpleSeededEvaluator()
