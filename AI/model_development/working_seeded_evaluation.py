...
class WorkingSeededEvaluator:
    """Working evaluation using existing metrics function with different seeds."""
    ...
    def save_results(self):
        ...
        json_file = self.output_dir / f"working_seeded_evaluation_detailed_{timestamp}.json"
        with open(json_file, 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        logger.info(f"💾 Detailed results saved to: {json_file}")
        
        # Save summary as CSV
        csv_file = self.output_dir / f"working_seeded_evaluation_summary_{timestamp}.csv"
...
def main():
    ...
    logger.info("🎲 Working Seeded Evaluation for Two-Stage Anomaly Detection")
    logger.info("="*60)
